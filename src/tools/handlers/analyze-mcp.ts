...
export async function handleAnalyzeMcp(args: unknown, baseUrl: string): Promise<string> {
  const { url } = args as { url?: string };
  
  const targetOrigin = url ? (url.startsWith("http") ? new URL(url).origin : new URL(url, new URL(baseUrl).origin).origin) : new URL(baseUrl).origin;
      ...
      try {
        ...
        if (parsed.mcpServers && typeof parsed.mcpServers === "object") {
          ...
          for (const [name, cfg] of servers.slice(0, 2)) {
            ...
            if (serverUrl) {
              const reachable = await fetchUrl(serverUrl);
              if (reachable && reachable.includes("jsonrpc")) {
                results.push(`    ✓ MCP endpoint REACHABLE (MCP response)`);
              } else if (reachable) {
                results.push(`    ✗ Endpoint exists but not MCP protocol`);
              } else {
                results.push(`    ✗ Endpoint NOT reachable`);
              }
            }
          }
