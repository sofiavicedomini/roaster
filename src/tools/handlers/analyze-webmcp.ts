...
export async function handleAnalyzeWebmcp(args: unknown, baseUrl: string): Promise<string> {
  ...
  if (parsed.tools && Array.isArray(parsed.tools)) checks.push(`tools: ${parsed.tools.length}`);
  if (parsed.prompts && Array.isArray(parsed.prompts)) checks.push(`prompts: ${parsed.prompts.length}`);
  if (parsed.resources && Array.isArray(parsed.resources)) checks.push(`resources: ${parsed.resources.length}`);
