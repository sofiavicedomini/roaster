...
export async function handleAnalyzeLlmsTxt(args: unknown, baseUrl: string): Promise<string> {
  const { url: providedUrl } = args as { url?: string };
  
  const origin = baseUrl ? new URL(baseUrl).origin : providedUrl ? new URL(providedUrl).origin : "";
  ...
  for (const attempt of urlsToTry) {
    ...
    const hasInstructions = lower.includes("## instructions") || lower.includes("system ") || lower.includes("system_prompt") || lower.includes("prompt");
    const hasApi = lower.includes("## api") || lower.includes("endpoint") || lower.includes("/v1/");
    const hasAuth = lower.includes("authorization") || lower.includes("authentication") || lower.includes("bearer");
    ...
    const hasSchema = lower.includes("openapi") || lower.includes("swagger") || lower.includes("schema");
