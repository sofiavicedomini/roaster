...
export async function handleAnalyzeAgentSkills(
  args: unknown,
  baseUrl: string,
): Promise<string> {
  ...
  const checks: string[] = [];
  if (lower.includes("skill") || lower.includes("capability"))
    checks.push("skills listed");
  if (lower.includes("tool") || lower.includes("function"))
    checks.push("tools listed");
  ...
  checks.push(
    `skills: ${skillCount}, tools: ${toolCount}, lines: ${lines.length}`,
  );
