...
export async function handleAnalyzeOauth(args: unknown, baseUrl: string): Promise<string> {
  ...
  if (Array.isArray(parsed.grant_types_supported)) {
    checks.push(`grants: ${(parsed.grant_types_supported as string[]).join(", ")}`);
  }
  ...
  if (Array.isArray(parsed.response_types_supported)) {
    checks.push(`response_types: ${(parsed.response_types_supported as string[]).join(", ")}`);
  }
