...
export async function handleAnalyzeAccessibility(
  args: unknown,
  baseUrl: string,
): Promise<string> {
  ...
  if (hasMain) ok.push("<main> landmark");
  else issues.push("no <main> landmark");
  if (hasNav) ok.push("<nav> landmark");
  else issues.push("no <nav> landmark");
  if (hasHeader) ok.push("<header>");
  else issues.push("no <header>");
  if (hasFooter) ok.push("<footer>");
  else issues.push("no <footer>");
  ...
  const decorativeImgs = allImgs.filter((img) =>
    /alt\s*=\s*["']\s*["']/i.test(img),
  );
  if (allImgs.length > 0) {
    if (imgsWithoutAlt.length > 0)
      issues.push(
        `${imgsWithoutAlt.length}/${allImgs.length} images missing alt`,
      );
    else
      ok.push(
        `all ${allImgs.length} images have alt (${decorativeImgs.length} decorative)`,
      );
  }
  ...
  const inputsWithAria = inputs.filter((inp) =>
    /aria-label(ledby)?\s*=/i.test(inp),
  );
  const labelTags = (html.match(/<label[^>]*for\s*=/gi) || []).length;
  if (inputs.length > 0) {
    const covered =
      inputsWithAria.length + Math.min(inputsWithId.length, labelTags);
    if (covered < inputs.length)
      issues.push(
        `${inputs.length - covered}/${inputs.length} inputs may lack labels`,
      );
  ...
  if (buttons.length > 0) {
    if (emptyButtons.length > 0)
      issues.push(
        `${emptyButtons.length}/${buttons.length} buttons have no accessible name`,
      );
  ...
  const emptyLinks = links.filter((link) => {
    ...
    return (
      inner.length === 0 &&
      !/aria-label\s*=/i.test(link) &&
      !/title\s*=/i.test(link)
    );
  });
  if (emptyLinks.length > 0)
    issues.push(
      `${emptyLinks.length} links with no accessible name (no text, no aria-label)`,
    );
  else if (links.length > 0)
    ok.push(`${links.length} links have accessible names`);
  ...
  const ariaHiddenCount = (html.match(/aria-hidden\s*=\s*["']true["']/gi) || [])
    .length;
  ...
  if (ariaLabelCount === 0 && ariaRoleCount === 0)
    issues.push("no ARIA attributes found");

  // --- tabindex abuse ---
  const badTabindex = (html.match(/tabindex\s*=\s*["'][1-9]\d*["']/gi) || [])
    .length;
  if (badTabindex > 0)
    issues.push(
      `${badTabindex} elements with tabindex > 0 (breaks natural tab order)`,
    );

  // --- Focus styles (inline hint) ---
  const hasFocusStyle =
    html.includes(":focus") || html.includes("focus-visible");
  ...
  if (disabledWithoutAria > 0 && ariaDisabled === 0) {
    issues.push(
      `${disabledWithoutAria} disabled element(s) without aria-disabled`,
    );
  }
