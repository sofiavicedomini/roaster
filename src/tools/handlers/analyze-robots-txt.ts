...
export async function handleAnalyzeRobotsTxt(args: unknown, baseUrl: string): Promise<string> {
  const { url: targetUrl } = args as { url: string };
  const origin = targetUrl.startsWith("http") ? new URL(targetUrl).origin : new URL(baseUrl).origin;
  ...
  for (const line of lines) {
    ...
    } else if (trimmed.startsWith("disallow:") || trimmed.startsWith("allow:") || trimmed.startsWith("crawl-delay:")) {
      rules.push(trimmed);
