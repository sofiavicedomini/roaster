...
export async function handleAnalyzeConversion(args: unknown, baseUrl: string): Promise<string> {
  ...
  if ((hasEmail && hasTel && hasNumber) || hasDate) signals.push("valid input types detected");
  else signals.push("no valid input types");

  if (html.match(/required/i) || html.match(/pattern\s*=/i) || html.match(/minlength/i) || html.match(/maxlength/i)) signals.push("required validation");
  else signals.push("no required validation");

  if (html.match(/font-weight:\s*bold/i) || html.match(/background.*#[0-9a-f]/i) || html.match(/color:\s*#[0-9a-f]/i)) signals.push("CTA styled");
  else signals.push("CTA not styled");

  if (html.match(/secure/i) || html.match(/ssl/i) || html.match(/encrypted/i) || html.match(/trust/i)) signals.push("trust signals");
