...
export async function handleAnalyzeHtmlStructure(args: unknown, baseUrl: string): Promise<string> {
  const html = await getHtmlForAnalysis(args as { html?: string; url?: string }, baseUrl);
  if (!html) return "No HTML or URL provided. Pass url to analyze HTML structure.";
  const tags: string[] = [];
  if (html.includes("<!DOCTYPE") || html.includes("<!doctype")) tags.push("doctype");
  ...
  const missing = ["doctype", "lang", "charset", "viewport", "meta description"].filter(t => !tags.includes(t));
