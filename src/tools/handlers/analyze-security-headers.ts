export async function handleAnalyzeSecurityHeaders(
  args: unknown,
  baseUrl: string,
): Promise<string> {
  ...
  if (!headers.some((h) => h.startsWith("content-security-policy:")))
    missing.push("Content-Security-Policy");
  if (!headers.some((h) => h.startsWith("strict-transport-security:")))
    missing.push("Strict-Transport-Security");
  if (!headers.some((h) => h.startsWith("x-content-type-options:")))
    missing.push("X-Content-Type-Options");
  if (!headers.some((h) => h.startsWith("x-frame-options:")))
    missing.push("X-Frame-Options");
  if (!headers.some((h) => h.startsWith("referrer-policy:")))
    missing.push("Referrer-Policy");
  if (!headers.some((h) => h.startsWith("permissions-policy:")))
