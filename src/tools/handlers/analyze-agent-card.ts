...
export async function handleAnalyzeAgentCard(args: unknown, baseUrl: string): Promise<string> {
  ...
  if (Array.isArray(parsed.skills)) checks.push(`skills: ${parsed.skills.length}`);
  ...
  if (Array.isArray(parsed.capabilities)) checks.push(`capabilities: ${parsed.capabilities.length}`);
