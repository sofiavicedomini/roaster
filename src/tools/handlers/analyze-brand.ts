...
export async function handleAnalyzeBrand(
  args: unknown,
  baseUrl: string,
): Promise<string> {
  ...
  if (html.includes("<svg") || (html.includes("<img") && html.includes("logo")))
    signals.push("logo");
  if (
    html.match(/class="[^"]*color[^"]*"/i) ||
    html.match(/style="[^"]*color:/i)
  )
    signals.push("colors");
  if (html.includes("font-family") || html.match(/<link[^>]*font/i))
  ...
  if (html.includes("header") && html.includes("footer"))
    signals.push("layout consistency");
