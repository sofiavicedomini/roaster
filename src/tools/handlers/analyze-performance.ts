...
export async function handleAnalyzePerformance(args: unknown, baseUrl: string): Promise<string> {
  ...
  if (resolved) {
    ...
    if (rendered) {
      ...
      if (t.domContentLoaded > 3000) browserSection += "DOMContentLoaded > 3s (heavy JS). ";
      if (t.load > 5000) browserSection += "Load > 5s (heavy page). ";
      if (r.scripts > 15) browserSection += `${r.scripts} JS requests is high. `;
      if (r.total > 80) browserSection += `${r.total} total resources is excessive. `;
    }
  }
  ...
  if (!html.includes('loading="lazy"') && !html.includes("loading='lazy'")) issues.push("no lazy loading on images");
  ...
  if (!html.includes("font-display") && html.includes("@font-face")) issues.push("@font-face without font-display");
  if (!html.includes('rel="preload"') && !html.includes("rel='preload'")) issues.push("no resource preloading");

  const htmlAnalysis = issues.length > 0
