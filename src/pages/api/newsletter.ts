...
export async function POST({ request }: APIContext) {
  ...
  if (!accessToken) {
    return new Response(JSON.stringify({ error: "Newsletter not configured" }), { status: 503, headers: { "Content-Type": "application/json" } });
  }
  ...
  if (!contactRes.ok && contactRes.status !== 409) {
    ...
    return new Response(JSON.stringify({ error: "Subscription failed" }), { status: 502, headers: { "Content-Type": "application/json" } });
  }

  // 2. If a list ID is configured, add the contact to it
  if (listId) {
    const vid = contactRes.status === 409
...
async function getContactVid(email: string, token: string): Promise<string | null> {
  try {
