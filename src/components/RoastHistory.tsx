  ...
  return (
    <div className="mt-8 pt-6 border-t border-orange-500/10">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-xs font-bold text-orange-400/50 uppercase tracking-widest">
          {t.history.title}
        </h2>
        <button
          onClick={clearAll}
          aria-label="Clear all analysis history"
          className="text-xs text-orange-300/40 hover:text-orange-300 transition-colors font-medium"
        >
          {t.history.clearAll}
        </button>
      </div>
      <div className="flex flex-col gap-2">
        {history.map((h, i) => (
          ...
          >
            <span
              className={`text-base font-bold tabular-nums shrink-0 px-2 py-1 rounded bg-orange-500/10 ${scoreColor(h.score)}`}
            >
              {h.score}
              <span className="text-sm font-normal opacity-60">/10</span>
            </span>
            <div className="flex-1 min-w-0">
              <div className="text-sm text-orange-100/70 truncate group-hover:text-orange-100 transition-colors">
                {h.url}
              </div>
              <div className="text-xs text-orange-300/45 truncate mt-0.5">
                {h.verdict}
              </div>
            </div>
            <span className="text-xs text-orange-300/35 shrink-0">
